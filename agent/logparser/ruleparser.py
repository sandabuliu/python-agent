...
class Branch(Rulebase):
    TYPE = dict
    OPERATION = {
        'contain': lambda a, b: str(b) in str(a),
        'not contain': lambda a, b: str(b) not in str(a),
        'null': lambda a, b: a is None or a == '',
        'not null': lambda a, b: a is not None and a != '',
        None: lambda a, b: a
    }

    def __init__(self, rule):
        Rule = globals()[rule['agent']['type'].lower().capitalize()]
        self.rule = Rule(rule['agent']['rule'])
        if not hasattr(self.rule, 'parse'):
            raise Exception('Unsupported rule %s!' % rule['agent'])
        self.names, self.operations, self.values = self.unpack(rule['name'])

    def unpack(self, name):
        if isinstance(name, basestring):
            return [name], [Branch.OPERATION[None]], [None]
        if isinstance(name, dict):
            return [name['name']], [Branch.OPERATION[name['operator']]], [name['value']]
        if isinstance(name, list):
            keys, operators, values = [], [], []
            for item in name:
                key, operator, value = self.unpack(item)
                keys += key
                operators += operator
                values += value
            return keys, operators, values
        raise Exception('Bad Branch type %s(%s)' % (name, type(name)))

    def parse(self, log):
        res = self.rule.parse(log)
        name = '_'.join([str(operator(res.get(self.names[i]), self.values[i]))
